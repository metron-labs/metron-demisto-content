...
from Tests.Marketplace.marketplace_constants import PackFolders, Metadata, GCPConfig, BucketUploadFlow, PACKS_FOLDER, \
    TRENDING_TAG_NAME, LANDING_PAGE_SECTIONS_PATH
...
class Pack(object):
    ...
    def _parse_pack_metadata(self, user_metadata, build_number, commit_hash):
        ...
        """
        pack_metadata = dict()
        pack_metadata['name'] = self._display_name or self._pack_name
        pack_metadata['id'] = self._pack_name
        pack_metadata['description'] = self._description or self._pack_name
        pack_metadata['created'] = self._create_date
        pack_metadata['updated'] = self._update_date
        pack_metadata['legacy'] = self._legacy
        pack_metadata['support'] = self._support_type
        pack_metadata['supportDetails'] = self._support_details
        pack_metadata['eulaLink'] = Metadata.EULA_URL
        pack_metadata['author'] = self._author
        pack_metadata['authorImage'] = self._author_image
        pack_metadata['certification'] = self._certification
        pack_metadata['price'] = self._price
        if self._is_private_pack:
            pack_metadata['premium'] = self._is_premium
            pack_metadata['vendorId'] = self._vendor_id
            pack_metadata['partnerId'] = self._partner_id
            pack_metadata['partnerName'] = self._partner_name
            pack_metadata['contentCommitHash'] = self._content_commit_hash
            pack_metadata['previewOnly'] = self._preview_only
        pack_metadata['serverMinVersion'] = user_metadata.get('serverMinVersion') or self.server_min_version
        ...
        pack_metadata['downloads'] = self._downloads_count

        # Setting a pack tags must come before calculating the pack's searchRank
        pack_metadata['tags'] = list(self._tags)
        pack_metadata['categories'] = self._categories
        pack_metadata['contentItems'] = self._content_items
        pack_metadata['searchRank'] = self._search_rank

        pack_metadata['integrations'] = self._related_integration_images
        pack_metadata['useCases'] = self._use_cases
        pack_metadata['keywords'] = self._keywords
        pack_metadata['dependencies'] = self._dependencies
    ...
    def _collect_pack_tags(self, user_metadata, landing_page_sections, trending_packs):
        ...
        tags |= {'TIM'} if self._is_feed else set()
        tags |= {'Use Case'} if self._use_cases else set()
    ...
    def _enhance_pack_attributes(self, user_metadata, index_folder_path, pack_was_modified, author_image,
        ...
        if statistics_handler:  # Public Content case
            self._pack_statistics_handler = statistics_handler.packs_map[self._pack_name]
        ...
        self._search_rank = PackStatisticsHandler.get_search_rank(
...
class PackStatisticsHandler:
    ...
    Attributes:
        pack_object (Tests.Marketplace.marketplace_services.Pack): The pack object.
    ...
    """
    def __init__(self, pack_object, packs_statistics_df: DataFrame, index_folder_path: str):
        self.pack_object = pack_object
        self.pack_name: str = pack_object.name
        ...
        self.download_count: int = PackStatisticsHandler._get_pack_downloads_count(self.pack_name, packs_statistics_df)

    @staticmethod
    def _get_pack_downloads_count(pack_name: str, packs_statistics_df: DataFrame):
        """ Returns number of packs downloads from big query dataframe.

        Args:
            pack_name: The pack name.
             packs_statistics_df: packs downloads statistics table.

        Returns:
            int: number of packs downloads.
        """
        downloads_count = 0
        if pack_name in packs_statistics_df.index.values:
            downloads_count = int(packs_statistics_df.loc[pack_name]['num_count'].astype('int32'))

        return downloads_count

    @staticmethod
    def get_search_rank(tags, certification, content_items):
...
class StatisticsHandler:
    """" A class that handles all packs statistics in the Upload-Flow.
    ...
    Attributes:
        ...
        packs_map (Dict[str, PackStatisticsHandler]): A mapping from pack name to its corresponding PackStatistics object.
    ...
    def __init__(self, service_account: str, index_folder_path: str, packs_list: list):
        ...
        self.packs_map: dict = {
            pack.name: PackStatisticsHandler(pack, self.packs_statistics_df, index_folder_path) for pack in packs_list
        }
    ...
    def get_landing_page_sections() -> Dict:
        ...
        return load_json(LANDING_PAGE_SECTIONS_PATH)
    ...
    def _filter_packs_from_before_3_months(self, pack_list_to_filter: list) -> List[str]:
        ...
        for pack_name in pack_list_to_filter:
            if Pack.pack_created_in_time_delta(pack_name, three_months_delta, self._index_folder_path):
